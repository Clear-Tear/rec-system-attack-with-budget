...
def s_test(z_test, t_test, model, z_loader, gpu=-1, damp=0.01, scale=25.0,
    ...
    for i in range(recursion_depth):
        ...
        for x, t in z_loader:
            ...
            y = model(x)
            loss = calc_loss(y, t)
            params = [ p for i,p in enumerate(model.parameters()) if p.requires_grad and i>=20 ]
...
def grad_z(z, t, model, gpu=-1):
    ...
    model.eval()
    # initialize
    # get: rec_prediction, trust_scores, targets
    if gpu >= 0:
        z, t = z.cuda(), t.cuda()
    rec_prediction = model(z)
    loss = calc_loss_multi(rec_prediction, t)
    # Compute sum of gradients from model parameters to loss
    params = [ p for i,p in enumerate(model.parameters()) if p.requires_grad and i>=20 ]
    return list(grad(loss, params, create_graph=True))

def calc_loss_multi(rec_prediction,t):
    rec_loss_function = nn.BCEWithLogitsLoss()
    loss1 = rec_loss_function(rec_prediction, trans_to_cuda(t.float()))
