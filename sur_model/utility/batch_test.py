import torch
import sur_model.utility.metrics as metrics
import multiprocessing
import heapq
import numpy as np
from sur_model.utility.gpuutil import trans_to_cpu, trans_to_cuda
...
def test_one_user(user,test_item,neg_item,model):
    ...
    useritem = to_useritem(torch.from_numpy(users).long(),torch.from_numpy(np.array(test_items)).long())
    predictions = model(useritem=trans_to_cuda(useritem.T),
                        slice_indices=None, trust_data=None, flag=1)
